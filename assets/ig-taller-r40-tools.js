...
(function(root,factory){
  ...
  function renderPhoto(host,state,changed){
    ...
    file.onchange=function(){var f=file.files&&file.files[0];if(!f)return;if(f.size>12*1024*1024){root.IGT.say(L('La imagen es demasiado grande.','The image is too large.'));return;}var r=new FileReader();r.onload=function(){state.dataUrl=String(r.result);state.name=f.name;img.onload=draw;img.src=state.dataUrl;changed();};r.readAsDataURL(f);};
