...
def measure(page, locator, required, state):
    ...
    for x,y,rw,rh in rects:
        ...
        for py in range(top,bottom,2):
            for px in range(left,right,2):
                ...
                vals.append(r)
                if r<worst['ratio']:
                    worst={'ratio':r,'x':px,'y':py,'background_rgb':list(bg)}
    if not vals:
        raise RuntimeError('No background pixels sampled')
    vals.sort()
    ratio01=p01(vals)
    return {
        'state':state,
                        ...
                        try:
                            ...
                            for idx in range(count):
                                ...
                                for state in target_states:
                                    try:
                                        m=measure(page,target,contract['normal_text_ratio'],state)
                                        rows.append({
...
def main():
    ...
    report={
        'schema':'W02_R1_CONTRAST_GATE_REPORT/1.0',
        'phase':args.phase,
