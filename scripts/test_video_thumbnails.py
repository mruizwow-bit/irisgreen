...
ID=re.compile(r'^[A-Za-z0-9_-]{11}REPORT={'cases':[],'failures':[],'notes':['Pruebas en Chromium con dominios externos bloqueados.','La existencia de una imagen no valida la disponibilidad o incrustación del vídeo.','Un vídeo puede usar el fallback visible «Miniatura no disponible» cuando aún no existe copia local revisada.','Se recorre la videoteca como visitante; no se fuerza el estado del componente.','La portada aprobada ya no contiene videoteca; sus miniaturas se prueban únicamente en /es/videos/.']}
...
with sync_playwright() as pw:
    ...
    for width,lang in [(1440,'es'),(390,'es'),(320,'es'),(390,'en')]:
        ...
        try:
            ...
            page.wait_for_function('(sel)=>{const im=document.querySelector(sel);return im&&im.complete&&im.naturalWidth>=320}',arg='main img.ig-video-thumbnail')
            ...
            for n in range(count):
                ...
                if ident in manifest.get('images',{}):
                    ...
                    fallback=poster.locator('.ig-thumbnail-unavailable');fallback.wait_for(state='visible')
            ...
            row['images_checked']=len(checked);row['correct_image_ids']=len(set(checked));row['fallbacks_checked']=len(fallbacks);row['fallback_ids']=sorted(fallbacks);row['image_size_stable']=True
