...
ESTRUCTURA = {
    ...
    "estado editorial en JSON": re.compile(r'"(?:status|estado|reviewed|validated|verificado|aprobado)"\s*:\s*"(?:borrador|draft|revisad[oa]|reviewed|validad[oa]|validated|verificad[oa]|verified|aprobad[oa]|approved|pending|pendiente|en revisi[óo]n)"', re.I),
    "revisión pública en JSON-LD": re.compile(r'"lastReviewed"\s*:|"reviewedBy"\s*:\s*\{', re.I),
