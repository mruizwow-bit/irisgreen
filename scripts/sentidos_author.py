#!/usr/bin/env python3
"""Import only the author's Sentidos descriptions; deployments CHECK, never rewrite.
...
SLUGS = [
 ...
 'el-ruido-le-resulta-insoportable', 'los-examenes-en-un-aula-ruidosa-son-insoportables',
 'los-sonidos-normales-me-resultan-dolorosos', 'hay-sonidos-concretos-que-me-provocan-una-reaccion',
 'las-luces-blancas-me-cansan-muy-rapido', 'el-olor-de-algunas-tiendas-me-obliga-a-salir',
 'no-noto-que-tengo-frio-hasta-que-estoy-temblando', 'el-calor-me-satura-antes-que-a-otras-personas',
 'necesito-moverme-para-poder-concentrarme', 'me-mareo-con-ciertos-movimientos-o-escaleras-mecanicas',
 'las-multitudes-me-desorientan-aunque-no-haya-mucho-ruido', 'el-contacto-inesperado-me-sobresalta-muchisimo',
 'me-cuesta-saber-cuanta-fuerza-estoy-usando', 'necesito-tocar-o-apretar-cosas-para-regularme',
 'algunos-sabores-me-resultan-demasiado-intensos', 'el-agua-de-la-ducha-me-molesta-en-la-piel',
 'los-cambios-de-temperatura-me-dejan-agotada', 'me-cuesta-encontrar-una-postura-comoda-para-sentarme',
 'la-ropa-que-tolero-un-dia-me-molesta-otro'
]
...
def load_source():
    ...
    if [int(x[0]) for x in blocks] != list(range(1,25)): raise ValueError('Expected all 24 complete ES/EN entries')
...
def search_update(text, rows):
    ...
    while True:
        ...
        if obj.get('u') in by_url:
            ...
            if obj.get('a')!='Sentidos' or obj['en']['u']!=route(row['en_path']): raise ValueError('Wrong search mapping')
            obj['t']=row['title_es'];obj['d']=row['es'];obj['en']['d']=row['en']
            patches.append((pos,end,json.dumps(obj,ensure_ascii=False,indent=2).replace('\n','\n  ')))
        pos=end
    if count!=24: raise ValueError('Missing search records')
...
def import_once():
    ...
    for row,slug in zip(rows,SLUGS):
        ...
        for lang,text in [('es',es),('en',en)]:
            ...
            # The entire body except the approved title/description must be byte-identical.
            def protected(s):
...
def approved_rows():
    ...
    for row,entry in zip(rows,m['entries']):
        ...
        row.update(entry)
    if len({r['es_path'] for r in rows})!=24 or len({r['en_path'] for r in rows})!=24:raise ValueError('Duplicate mapping')
    return rows
...
def check(root):
    ...
    for row in rows:
        for lang in ['es','en']:
            ...
            checked.append(path)
    return {'entries':24,'exact_descriptions':48,'spanish_titles':24,'english_titles':'preserved; absent from submitted source','checked_pages':checked,'catalogue_cards':48,'search_entries':24,'source_sha256':SOURCE_SHA256,'mode':'read-only verification; no automatic rewriting'}
