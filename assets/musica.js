...
(function () {
  ...
  var panel, audio, playButton, title, author, status, closeButton, lastTrigger, labels, selected = 0, repeat = true, open = false;
  ...
  function create() {
    ...
    var css=el('style');css.id='ig-music-styles';css.textContent=
      ...
      '#ig-music-panel .ig-m-volume{display:flex;align-items:center;gap:8px;margin:6px 0;font-size:13px}#ig-music-panel input[type=range]{flex:1;min-width:0;min-height:30px}#ig-music-panel summary{cursor:pointer;padding:12px 0;min-height:44px;font-weight:700}' +
      ...
      '#ig-music-panel .ig-m-repeat{font-size:13px;display:flex;align-items:center;gap:6px;min-height:36px}#ig-music-panel .ig-m-credit{font-size:12px;color:#5a6675;margin:8px 0 0}#ig-music-panel .ig-m-status{font-size:13px;color:#8c245a;margin:4px 0}#ig-music-panel .ig-m-status:empty{display:none}';
    ...
    controls.append(button(labels.prev,function(){play(selected-1);}),playButton,button(labels.next,function(){play(selected+1);}));panel.append(controls);
    ...
    slider.addEventListener('input',function(){ensureAudio().volume=Number(slider.value)/100;});volume.append(el('span',labels.volume),slider);panel.append(volume);
    var details=el('details'), summary=el('summary',labels.list),list=el('ol');
    ...
    var loopLabel=el('label','', 'ig-m-repeat'),loop=el('input');loop.type='checkbox';loop.checked=true;loop.addEventListener('change',function(){repeat=loop.checked;});loopLabel.append(loop,el('span',labels.repeat));panel.append(loopLabel);
    status=el('p','', 'ig-m-status');status.setAttribute('role','status');panel.append(status,el('p',labels.credit,'ig-m-credit'));
  ...
  document.addEventListener('click',function(event){
    ...
    if(!panel)create();
    labels=language();panel.querySelector('#ig-music-title').textContent=labels.title;
    closeButton.setAttribute('aria-label',labels.close);sync();panel.hidden=false;
