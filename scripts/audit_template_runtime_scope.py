...
def main() -> None:
    ...
    for path in sorted(root.rglob("*.html")):
        ...
        if not active_expr and not runtime_refs and not X_DC.search(active): continue
        rows.append({
    ...
    report = {
        ...
        "runtime_reference_counts": {runtime: sum(runtime in r["runtime_refs"] for r in rows) for runtime in RUNTIMES},
        "pages": rows,
    }
    print(json.dumps(report, ensure_ascii=False, indent=2))
    if debt:
        raise AssertionError("Quedan plantillas {{ }} activas en el HTML inicial: " + ", ".join(r["route"] for r in debt[:20]))
