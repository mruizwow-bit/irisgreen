#!/usr/bin/env python3
"""Cierra la ficha pública ES/EN de discalculia/disgrafía sin inventar prevalencia.

La salida final debe explicar la incertidumbre metodológica con lenguaje público,
conservar fuentes verificables y no exponer rótulos del proceso editorial.
...
'marker':'Sin cifra mundial única seleccionada','heading':'Qué podemos afirmar',
...
'forbidden':('La conclusión publicable','decisión editorial','No rellenes este bloque')},
'en/data/dyscalculia-and-dysgraphia-why-we-do-not-give-a-single-global-figure/index.html':{
'marker':'No single global figure selected','heading':'What we can say',
...
'forbidden':('The publishable conclusion','editorial decision','Do not fill this block')}}

def ensure_copy(text,old,new,rel):
    if new in text:return text
    if old in text:return text.replace(old,new,1)
    raise ValueError(f'No se encuentra ni la redacción antigua ni la final en {rel}: {new[:80]}')

def ensure_heading(text,cfg,rel):
    # El limpiador de estados puede haber cambiado o retirado el rótulo antes de este paso.
    # Identificamos la sección por su cifra central, no por un rótulo intermedio.
    pattern=re.compile(r'(<section class="sec"><h2>).*?(</h2><div class="cifra-b"><span class="n">'+re.escape(cfg['marker'])+r'</span>)',re.S)
    text,n=pattern.subn(lambda m:m.group(1)+cfg['heading']+m.group(2),text,count=1)
    if n!=1 and cfg['heading'] not in text:
        raise ValueError(f'No se localiza la sección central en {rel}')
    return text
...
def main():
    ...
    for rel,cfg in PAGES.items():
        ...
        old_text=p.read_text(encoding='utf-8');text=old_text
        text=ensure_copy(text,cfg['old_intro'],cfg['new_intro'],rel)
        text=ensure_heading(text,cfg,rel)
        text=ensure_copy(text,cfg['old_conclusion'],cfg['new_conclusion'],rel)
        text=ensure_copy(text,cfg['old_cite'],cfg['new_cite'],rel)
        text=ensure_copy(text,cfg['old_avoid'],cfg['new_avoid'],rel)
        ...
        required=(cfg['heading'],cfg['new_intro'],cfg['new_conclusion'],cfg['new_cite'],cfg['new_avoid'])
        missing=[x for x in required if x not in text]
        forbidden=[x for x in cfg['forbidden'] if x in text]
        if missing or forbidden:
            raise AssertionError(f'Salida pública incorrecta en {rel}; faltan={missing}; sobran={forbidden}')
        if text!=old_text:p.write_text(text,encoding='utf-8');changed.append(rel)
    print({'paginas_actualizadas':changed,'fuentes':['PMID 38929203','PMID 42470314'],'lenguaje_interno_restante':0})
