...
function poster(k){if(!stage)return;stage.innerHTML='';var f=document.createElement('figure');f.className='r40-static-fallback';if(POST[k]){var im=document.createElement('img');im.src=POST[k];im.alt='';f.appendChild(im);}else{var cv=document.createElement('canvas');cv.setAttribute('role','img');cv.setAttribute('aria-label',D[k]);f.appendChild(cv);if(window.IGOctopusScene)f._oct=window.IGOctopusScene.create(cv,{reduced:function(){return true;}});}var cap=document.createElement('figcaption');cap.textContent=D[k];f.appendChild(cap);stage.appendChild(f);}
