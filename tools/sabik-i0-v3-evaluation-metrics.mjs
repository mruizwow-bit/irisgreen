...
function actionDiff(expected,predicted){
  const usedExpected=new Set(),usedPredicted=new Set(),wrongPairs=[];
  for(let i=0;i<expected.length;i++){
    const e=JSON.stringify(expected[i]);
    const j=predicted.findIndex((p,k)=>!usedPredicted.has(k)&&JSON.stringify(p)===e);
    if(j>=0){usedExpected.add(i);usedPredicted.add(j);}
  }
  for(let i=0;i<expected.length;i++){
    if(usedExpected.has(i))continue;
    const j=predicted.findIndex((p,k)=>!usedPredicted.has(k)&&p.type===expected[i].type);
    if(j>=0){usedExpected.add(i);usedPredicted.add(j);wrongPairs.push({expected:expected[i],predicted:predicted[j]});}
  }
  const omitted=expected.filter((_,i)=>!usedExpected.has(i));
  const extra=predicted.filter((_,i)=>!usedPredicted.has(i));
  return {omitted,extra,wrong:wrongPairs.length,wrong_pairs:wrongPairs};
}
...
export function evaluateI0Cases(cases,predictions){
  ...
  const multi={total:0,command_exact:0,action_exact:0,full_exact:0,command_exact_cases:0,action_exact_given_command_exact:0,omitted:0,extra:0,wrong:0,order_errors:0,omitted_when_command_exact:0,extra_when_command_exact:0,wrong_when_command_exact:0,order_errors_when_command_exact:0};
  ...
  for(const c of cases){
    ...
    if(c.expected_commands.length>1){
      ...
      const orderError=nav>=0&&nav!==p.predicted_actions.length-1;
      if(commandsOk){
        multi.command_exact_cases++;
        multi.action_exact_given_command_exact+=Number(actionsOk);
        multi.omitted_when_command_exact+=d.omitted.length;
        multi.extra_when_command_exact+=d.extra.length;
        multi.wrong_when_command_exact+=d.wrong;
        multi.order_errors_when_command_exact+=Number(orderError);
      }
      if(orderError)multi.order_errors++;
    }
  ...
  return {
    ...
    multi_action:{...multi,action_exact_given_command_exact_rate:div(multi.action_exact_given_command_exact,multi.command_exact_cases),structural_gate_pass:multi.omitted_when_command_exact===0&&multi.extra_when_command_exact===0&&multi.order_errors_when_command_exact===0},
