# BLOQUE OBLIGATORIO EMBEBIDO · LEER ANTES DE EJECUTAR

**ESTE BLOQUE SE COPIA FÍSICAMENTE DENTRO DE LA ORDEN. NO SE SUSTITUYE POR UN ENLACE.**

Antes de ejecutar:
1. leer este bloque completo;
2. leer Control Maestro;
3. leer Memoria Maestra;
4. leer la fuente exacta del carril;
5. leer la orden específica;
6. publicar `R42_NORMATIVA_EMBEBIDA_LEIDA`;
7. construir en la misma sesión;
8. actualizar Memoria + Control al terminar.
...
# R42 · ARQUITECTURA DE CONTENIDO SEGURO POR ETAPA · SITUACIONES + CONDICIONES

## Motivo

María identifica un riesgo real: una persona menor puede entrar desde Juegos/Recursos y, mediante navegación, búsqueda o enlaces relacionados, llegar accidentalmente a contenido de alta sensibilidad (por ejemplo autolesión, suicidio, abuso, trauma o contenido alimentario potencialmente dañino).

La solución NO es ocultar indiscriminadamente temas importantes a menores. Algunos temas —por ejemplo acoso, abuso, consentimiento, pedir ayuda o salud mental— pueden ser esenciales para un niño o adolescente. La arquitectura debe distinguir:
1. **etapa de vida**; y
2. **sensibilidad del contenido**.

## Evidencia actual del producto

La web actual expone:
- **185 fichas de Condiciones** en un catálogo abierto;
- **187 Situaciones** en un catálogo abierto;
- el índice común `buscador.json` no contiene todavía metadatos de etapa/sensibilidad/discovery;
- Condiciones incluye actualmente temas de distinta sensibilidad en el mismo catálogo (por ejemplo abuso y explotación, anorexia nerviosa, TCA, depresión, TEPT, sexualidad, etc.).
...
No se pide fecha de nacimiento ni identidad.

### Sensibilidad independiente de la edad
- `S0_GENERAL`: contenido cotidiano/general.
- `S1_SENSITIVE`: contenido sensible que puede ser útil a menores con redacción adaptada.
- `S2_HIGH_SENSITIVITY`: contenido que no debe aparecer de forma incidental a menores (autolesión/suicidio, violencia o abuso con detalle, contenido alimentario de riesgo, trauma grave u otros equivalentes tras revisión humana).

### Discovery
- `NORMAL`: puede aparecer en catálogo, relacionados y búsqueda.
- `INTENTIONAL_ONLY`: nunca se recomienda incidentalmente; aparece solo ante búsqueda/intención clara o acceso directo.
- `SAFE_VARIANT_REQUIRED`: menores reciben versión adaptada/segura, no la versión completa.

## COMPORTAMIENTO

### Infancia
- Catálogos muestran S0 + S1 aprobados para infancia.
- S2 no aparece en recomendaciones, “también puede interesarte”, enlaces desde juegos, tarjetas aleatorias ni autocompletado.
- Si un tema S2 es importante para pedir ayuda, se ofrece una **versión segura para infancia**: explicación breve, no gráfica, sin métodos, centrada en seguridad y pedir apoyo.
- Un acceso directo a una ficha S2 no debe soltar de golpe la versión completa.

### Adolescencia
- S0/S1 disponibles con adaptación adolescente.
- S2: accesible cuando existe intención clara, con aviso neutral y contenido seguro; nunca recomendado algorítmicamente/incidentalmente.
- No métodos, no detalles gráficos, no glamour/normalización, no “rabbit holes”.

### Adultez
- Catálogo completo, manteniendo normas de redacción segura en contenidos de autolesión/suicidio.

### Cualquier edad / sin selección
- **safe-by-default**: navegación incidental equivalente a protección de menores.
- S2 no se promociona ni recomienda.
- Sigue siendo posible encontrar ayuda mediante búsqueda intencional y rutas de apoyo.

## PRINCIPIO FUNDAMENTAL

**“Sensible” no significa automáticamente “solo adultos”.**

No esconder a un niño una vía para pedir ayuda por acoso, abuso o miedo. Crear una versión adecuada a su etapa.

## PRIVACIDAD

Preferencia de etapa:
- se formula como “¿Para quién buscas?” o “Contenido para…”, no “¿Qué edad tienes?”;
- no requiere cuenta;
- no necesita DOB;
- puede mantenerse solo en sesión o localmente tras acción explícita;
- no se envía a servidor;
- no se usa para perfilar.

## BÚSQUEDA Y ENLACES

Toda búsqueda, buscador global, tarjetas relacionadas y enlaces cruzados deben consultar los metadatos `audience/sensitivity/discovery`.

Un juego marcado INFANCIA no puede enlazar incidentalmente a S2.
Taller/Intereses/Rincón respetan la misma lente al proponer contenido.

## FUENTES DE DISEÑO/SEGURIDAD

Como referencia de diseño y seguridad, no como afirmación automática de aplicabilidad jurídica:
- ICO Children’s Code: best interests, age-appropriate application, high privacy/default and data minimisation.
- AEPD: la protección de menores no exige que el proveedor conozca identidad o edad exacta; la garantía de edad debe ser proporcional y minimizada.
- Comisión Europea, directrices DSA de protección de menores: safety/privacy by design y mitigación de exposición a contenido nocivo.
- Samaritans: jóvenes son especialmente vulnerables ante contenido de autolesión/suicidio; reducir acceso a contenido dañino y evitar detalles/métodos, manteniendo vías de ayuda.

## Estado
`R42_CHILD_SAFE_CONTENT_ARCHITECTURE_REQUIRED`.
