/* R25 revision: N16 verifies decorative B3; localized functional names stay unchanged. */
...
async function main() {
  ...
  await contract('N16', 'English accessible names, labels, placeholder and effective language are coherent', async () => {
    ...
    assert.equal(await el('hologram').getAttribute('aria-label'), null);
    assert.equal(await el('hologram').getAttribute('aria-hidden'), 'true');
