...
(function(root,factory){
  ...
  function createTopbar(main,app,study){
    ...
    var center=el('div','ig42-topbar-center');center.appendChild(stageSelector(study));top.appendChild(center);
    ...
    var challenge=btn(T('Reto','Challenge'),'ig42-action-btn');var file=btn(T('Archivo','File'),'ig42-action-btn');var help=btn(T('Ayuda','Help'),'ig42-icon-btn');help.setAttribute('aria-label',T('Ayuda','Help'));help.textContent='?';
    actions.appendChild(undo);actions.appendChild(redo);actions.appendChild(challenge);actions.appendChild(file);actions.appendChild(help);top.appendChild(actions);
    main.insertBefore(top,app);
    return {node:top,undo:undo,redo:redo,challenge:challenge,file:file,help:help,state:state};
  }
  ...
  function microPreview(kind){var p=el('div','ig42-micro ig42-micro-'+(kind||'existing'));p.setAttribute('aria-hidden','true');for(var i=0;i<8;i++)p.appendChild(el('span',''));return p;}
