...
assert.ok(ctl.includes("list.remove()")&&ctl.includes("row.remove()"),'HOLD recordings are removed from the live R40 DOM');
...
assert.ok(calm.includes("var SOUNDS = $('#r40StartAV') ? [] : ["),'historical recording mixer is disabled in R40');
assert.ok(es.includes('id="audioCredit">Creado en Iris Green · no es una grabación</div>'),'ES visible audio credit is first-party');
assert.ok(en.includes('id="audioCredit">Made by Iris Green · not a recording</div>'),'EN visible audio credit is first-party');
