...
def global_csp(root):
    text=(root/'_headers').read_text(encoding='utf-8');m=re.search(r'^/\*\s*$.*?^\s*Content-Security-Policy:\s*(.+)$',text,re.M|re.S)
    if not m:raise RuntimeError('No se encuentra la CSP global en _headers')
...
def csp_console_bag(page):
    bag=[];page.on('console',lambda msg:bag.append(msg.text) if ('Content Security Policy' in msg.text or 'Refused to' in msg.text) else None);return bag

def expand_video_collection(page):
...
def test_video_frames(browser,base,failures):
    tested=[];html=(ROOT/'es/videos/index.html').read_text(encoding='utf-8',errors='strict')
    for _,source,_ in VIDEO_PROVIDERS:
        if source not in html:failures.append(f'/es/videos/: el componente ya no declara el proveedor revisado {source}')
    for label,source,required_card in VIDEO_PROVIDERS:
        page=browser.new_page(viewport={'width':1280,'height':900});page.set_default_timeout(15000);messages=csp_console_bag(page);attempts=[]
        def abort_provider(route,request,bag=attempts):bag.append(request.url);route.abort()
        page.route(source+'/**',abort_provider)
        try:
            page.goto(base+'/es/videos/',wait_until='domcontentloaded');page.locator('main h1').first.wait_for(state='visible');expand_video_collection(page)
            poster=page.locator(f'button.ig-video-poster[data-ig-video^="{source}"]').first
            if not poster.count():
                if required_card:failures.append(f'/es/videos/: no queda ninguna tarjeta activa para probar {label}')
                continue
            poster.scroll_into_view_if_needed();poster.click();frame=page.locator(f'main iframe[src^="{source}"]').first;frame.wait_for(state='attached');page.wait_for_timeout(150)
            if not attempts:failures.append(f'/es/videos/: {label} no llegó a intentar la carga; revisar frame-src')
            if messages:failures.extend(f'/es/videos/ {label}: {x}' for x in messages[:6])
            tested.append(label)
        except Exception as exc:failures.append(f'/es/videos/ {label}: {exc}')
        finally:page.close()
    return tested
    ...
    try:
        with sync_playwright() as pw:
            ...
            for route,expected in ROUTES:
                page=browser.new_page(viewport={'width':1280,'height':900});page.set_default_timeout(15000);csp_messages=csp_console_bag(page);request_failures=[]
                page.on('requestfailed',lambda req,bag=request_failures:bag.append(f'{req.url} :: {req.failure}'))
                try:
                    page.goto(base+route,wait_until='domcontentloaded');page.locator('main h1').first.wait_for(state='visible');page.wait_for_timeout(500);h1=page.locator('main h1').first.inner_text().strip();body=page.locator('body').inner_text()
                    if expected.casefold() not in (h1+' '+body).casefold():failures.append(f'{route}: no aparece texto esperado {expected!r}; h1={h1!r}')
                    if route=='/' and page.locator('#q').is_disabled():failures.append('/: el buscador sigue deshabilitado con CSP')
                    blocked=[x for x in request_failures if 'ERR_BLOCKED_BY_CSP' in x or ('blocked' in x.lower() and 'csp' in x.lower())]
                    if csp_messages:failures.extend(f'{route}: {x}' for x in csp_messages[:6])
                    if blocked:failures.extend(f'{route}: {x}' for x in blocked[:6])
