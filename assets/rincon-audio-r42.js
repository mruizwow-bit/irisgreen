...
function make(id){var sr=24000,dur=12,n=sr*dur,L=new Float32Array(n),R=new Float32Array(n),r=rng(hash(id)),i,t;
 ...
 else if(id==='rio'){bed(500,190,.014,.92);for(i=0;i<34;i++){t=r()*11.7;addNoiseEvent(L,R,sr,t,.18+r()*.42,520+i,320,.0015+r()*.0032,r()*1.2-.6);if(r()<.28)addTone(L,R,sr,t,.16+r()*.22,95+r()*90,150+r()*120,.0007+r()*.0012,r()*1.0-.5);}}
 ...
 else if(id==='escena-rio'){bed(910,170,.012,.94);for(i=0;i<32;i++){t=r()*11.7;addNoiseEvent(L,R,sr,t,.20+r()*.44,920+i,280,.0013+r()*.0028,r()*1.1-.55);}for(i=0;i<8;i++){t=.6+i*1.45+r()*.25;addTone(L,R,sr,t,.22,78+r()*34,112+r()*50,.0006,r()*.8-.4);}}
