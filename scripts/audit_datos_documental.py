...
No verifica todavía que cada cifra coincida con la fuente externa. Comprueba que cada
ficha publicada declara lo necesario para poder verificarla después: fuente enlazada,
población, año, método, publicación y límites de interpretación.
"""
...
def main() -> None:
    ...
    for path in pages:
        ...
        # Debe existir al menos una limitación explícita en Qué mide o Qué evitar.
        limitation_text = plain(sections.get('qué mide', '') + ' ' + sections.get('qué evitar', '')).casefold()
        limit_terms = ('no ', 'no debe', 'no significa', 'no permite', 'limit', 'puede reflejar', 'asociación', 'causa')
        if not any(term in limitation_text for term in limit_terms):
            problems.append('sin límite de interpretación detectable')
