#!/usr/bin/env python3
"""Prueba en Chromium las 9 interfaces DC públicas vigentes tras la migración sin unsafe-eval."""
from __future__ import annotations
import functools,re,threading
from http.server import SimpleHTTPRequestHandler,ThreadingHTTPServer
from pathlib import Path
from playwright.sync_api import sync_playwright
ROOT=Path.cwd().resolve();SAFE_RUNTIME="/assets/runtime/dc-runtime-csp.js"

def global_csp():
    text=(ROOT/'_headers').read_text(encoding='utf-8',errors='strict')
    match=re.search(r'^/\*\s*$.*?^\s*Content-Security-Policy:\s*(.+)$',text,re.M|re.S)
    if not match:raise RuntimeError('No se encuentra la CSP global')
...
def server(csp):
    ...
    threading.Thread(target=httpd.serve_forever,daemon=True).start();return httpd,f'http://127.0.0.1:{httpd.server_port}'
...
def main():
    ...
    if len(pages)!=9:raise AssertionError(f'Inventario DC seguro cambiado: {len(pages)} != 9')
    csp=global_csp()
    ...
    try:
        with sync_playwright() as pw:
            ...
            for path,route in pages:
                page=browser.new_page(viewport={'width':1280,'height':900});page.set_default_timeout(15000)
                ...
                try:
                    ...
                    if page.locator('x-dc').count():raise AssertionError('x-dc no fue sustituido por #dc-root')
                    body_text=page.locator('body').inner_text()
                    if '{{' in body_text or '}}' in body_text:raise AssertionError('quedan expresiones de plantilla visibles')
                    button=page.locator('main button:visible').first;interacted=False
                    if button.count() and button.is_enabled():
                        button.scroll_into_view_if_needed();button.click();page.wait_for_timeout(80);interacted=True
                    if page_errors:raise AssertionError('pageerror: '+' | '.join(page_errors[:4]))
                    bad=[x for x in console_errors if 'favicon' not in x.lower() and '404' not in x.lower()]
                    if bad:raise AssertionError('console: '+' | '.join(bad[:4]))
    ...
    if failures:raise AssertionError('Fallos DC CSP-safe:\n'+'\n'.join(failures[:30]))
    print({'pages_checked':len(checked),'unsafe_eval':False,'raw_templates_visible':0,'pages_with_basic_interaction':sum(x['interaction'] for x in checked),'routes':[x['route'] for x in checked]})
