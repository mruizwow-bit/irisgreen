...
ROOT=Path.cwd();OUT=ROOT/'reports'/'wcag-browser';OUT.mkdir(parents=True,exist_ok=True)
ROUTES=['/','/es/situaciones/','/es/situaciones/la-ropa-me-molesta/','/en/situations/clothes-feel-unbearable/','/es/neurodiversidad/condiciones/','/es/neurodiversidad/condiciones/autismo/','/en/neurodiversity/conditions/autism/','/es/biblioteca/','/es/investigacion/','/es/datos/','/es/datos/autismo-en-la-poblacion/','/en/data/autism-in-the-population/','/es/tramites/directorio/','/es/libros/','/es/videos/','/es/recursos/juegos/','/es/recursos/juegos/las-cinco-cosas/','/es/intereses/','/es/taller/','/es/sitio-tranquilo/']
TEXT_SPACING='''*:not(svg):not(svg *){letter-spacing:.12em!important;word-spacing:.16em!important;line-height:1.5!important}p{margin-bottom:2em!important}'''
...
JS_CLIPPED='''() => Array.from(document.querySelectorAll('body *')).filter(el=>{
 ...
 if(!direct&&!/^(INPUT|TEXTAREA|SELECT|BUTTON)$/.test(el.tagName))return false;
 const ox=cs.overflowX,oy=cs.overflowY;
 return ((ox==='hidden'||ox==='clip')&&el.scrollWidth>el.clientWidth+2)||((oy==='hidden'||oy==='clip')&&el.scrollHeight>el.clientHeight+2);
}).slice(0,12).map(el=>({tag:el.tagName,id:el.id,cls:String(el.className||'').slice(0,80),text:(el.textContent||el.value||el.placeholder||'').trim().slice(0,100),sw:el.scrollWidth,cw:el.clientWidth,sh:el.scrollHeight,ch:el.clientHeight}))'''
...
JS_OUTSIDE='''() => Array.from(document.querySelectorAll('body *')).filter(el=>{
 const cs=getComputedStyle(el),r=el.getBoundingClientRect();
 if(cs.display==='none'||cs.visibility==='hidden'||r.width<2||r.height<2||el.closest('[hidden],[aria-hidden="true"]'))return false;
  ...
  if route in ['/','/es/situaciones/','/es/neurodiversidad/condiciones/','/es/tramites/directorio/','/es/libros/']:
   ...
   def keyboard():
    ...
    for _ in range(12):
     ...
     assert f['ok'],f
    assert len(set(seen))>=4,seen;assert row['steps'][0]['tag']=='A' and ('contenido' in row['steps'][0]['text'].lower() or 'content' in row['steps'][0]['text'].lower()),row['steps'][0]
   add(row,keyboard)
  ctx.close()
 browser.close()
server.shutdown()
report['summary']={'cases':len(report['cases']),'passed':sum(c.get('passed',False) for c in report['cases']),'failures':len(report['failures'])};report['passed']=not report['failures']
